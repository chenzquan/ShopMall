...
public class UserServiceImpl implements IUserService {
     ...
     * 判断用户名 或 email 是否存在
     * @param str
     * @param type  用户名 或 email 类型
     * @return 校验username 或 email 是否成功
     */
  ...
  //      String md5Password = password;
    ...
    public ServerResponse<String> checkAnswer(String username, String question, String answer) {
        ...
        if(resultCount > 0){
            ...
            RedisPoolUtil.setEx(Const.TOKEN_PREFIX + username,forgetToken,60*30); //三十分钟
    ...
    public ServerResponse<String> resetPassword(String passwordOld, String passwordNew, User user) {
        //防止横向越权,要校验一下这个用户的旧密码,一定要指定是这个用户.因为我们会查询一个count(1),如果不指定id,那么结果就是true啦count==1;
