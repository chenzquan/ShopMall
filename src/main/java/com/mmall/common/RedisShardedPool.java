...
//        pool = new JedisPool(config,redisIp,redisPort, 1000*2);  //两分钟
        ...
        pool = new ShardedJedisPool(config,jedisShardInfoList, Hashing.MURMUR_HASH, Sharded.DEFAULT_KEY_TAG_PATTERN);//Hashing.MURMUR_HASH 策略
    ...
    public static ShardedJedis getJedis(){
        return pool.getResource(); //从连接池中 取一个实例
    }

    public static void returnResource(ShardedJedis jedis){  //把redis 给回 连接池
